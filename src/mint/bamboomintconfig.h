 ...
 *
 */
#ifndef _FCITX5_BAMBOO_BAMBOOMINTCONFIG_H_
#define _FCITX5_BAMBOO_BAMBOOMINTCONFIG_H_
    ...
    void dumpDescription(RawConfig &config) const {
        ...
        for (size_t i = 0; i < list_.size(); i++) {
            config.setValueByPath(
                ...
                stringutils::concat("fcitx://config/addon/bamboomint/macro/",
                                    list_[i]));
        }
    }
};
...
FCITX_CONFIGURATION(BambooMintKeymap,
                    Option<std::string> key{this, "Key", _("Key"), ""};
                    Option<std::string> value{this, "Value", _("Value"), ""};);

FCITX_CONFIGURATION(
    BambooMintMacroTable,
    OptionWithAnnotation<std::vector<BambooMintKeymap>, ListDisplayOptionAnnotation>
        macros{this,
               "Macro",
               _("Macro"),
               {},
...
FCITX_CONFIGURATION(
    BambooMintConfig, KeyListOption restoreKeyStroke{this,
                                                 "RestoreKeyStroke",
    ...
    SubConfigOption custumKeymap{this, "CustomKeymap", _("Custom Keymap"),
                                 "fcitx://config/addon/bamboomint/custom_keymap"};);
} // namespace fcitx
