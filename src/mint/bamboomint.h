 ...
 *
 */
#ifndef _FCITX5_BAMBOO_BAMBOOMINT_H_
#define _FCITX5_BAMBOO_BAMBOOMINT_H_
...
class BambooMintState;

class BambooMintEngine final : public InputMethodEngine {
public:
    BambooMintEngine(Instance *instance);
    ...
    void refreshEngine();
    void refreshOption();
    void saveConfig() { safeSaveAsIni(config_, "conf/bamboomint.conf"); }
...
private:
    Instance *instance_;
    BambooMintConfig config_;
    BambooMintCustomKeymap customKeymap_;
    std::unordered_map<std::string, BambooMintMacroTable> macroTables_;
    std::unordered_map<std::string, CGoObject> macroTableObject_;
    FactoryFor<BambooMintState> factory_;
...
class BambooMintFactory : public AddonFactory {
public:
    AddonInstance *create(AddonManager *manager) override {
        registerDomain("fcitx5-bamboo-mint", FCITX_INSTALL_LOCALEDIR);
        return new BambooMintEngine(manager->instance());
    }
};
} // namespace fcitx

#endif // _FCITX5_BAMBOO_BAMBOOMINT_H_
